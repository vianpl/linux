...
static int brcm_pcie_set_inbound_viewport(struct brcm_pcie *pcie)
{
	 ...
	 * addition, its size mut be a power of two. Further, the MSI target
	 * address must NOT be placed inside this region, as the decoding logic
	 * will consider its address to be inbound memory traffic.  To further
	 * complicate matters, the viewport must start on a pcie-address that
	 * is aligned on a multiple of its size.  If a portion of the viewport
	 * does not represent system memory -- e.g. 3GB of memory requires a
	 * 4GB viewport -- we can map the outbound memory in or after 3GB and
	 * even though the viewport will overlap the outbound memory the
