...
bool __read_mostly enable_apicv = false;
...
int kvm_set_msr_common(struct kvm_vcpu *vcpu, struct msr_data *msr_info)
{
	...
	case MSR_KVM_PV_EOI_EN:
		...
		if (kvm_lapic_set_pv_eoi(vcpu->arch.apic, data, sizeof(u8)))
			return 1;
		break;
 ...
 */
static void kvm_pv_kick_cpu_op(struct kvm_vcpu *vcpu, int apicid)
{
	/*
...
static void dump_ftrace_vcpu_state_events(struct kvm_vcpu *vcpu)
{
	...
	trace_printk("exc_payload: has_payload=%u payload=%llu\n",
...
static void dump_ftrace_vcpu_regs(struct kvm_vcpu *vcpu)
{
	...
	trace_printk("rax=%llu rbx=%llu rcx=%llu rdx=%llu\n", regs.rax,
		     regs.rbx, regs.rcx, regs.rdx);

	trace_printk("rsi=%llu rdi=%llu rsp=%llu rbp=%llu\n", regs.rsi,
		     regs.rdi, regs.rsp, regs.rbp);

	trace_printk("r8=%llu  r9=%llu  r10=%llu r11=%llu\n", regs.r8, regs.r9,
		     regs.r10, regs.r11);

	trace_printk("r12=%llu r13=%llu r14=%llu r15=%llu\n", regs.r12,
		     regs.r13, regs.r14, regs.r15);

	trace_printk("rip=%llu rflags=%llu\n", regs.rip, regs.rflags);
}

static void print_segment(const char *name, const struct kvm_segment seg)
{
	trace_printk("%s: base=%llu limit=%u selector=%u type=%u present=%u dpl=%u db=%u s=%u l=%u g=%u avl=%u unusable=%u\n",
...
{
	trace_printk("%s: base=%llu limit=%u\n", name, dtable.base, dtable.limit);
}
...
static void dump_ftrace_vcpu_sregs2(struct kvm_vcpu *vcpu)
{
	...
	trace_printk("cr0=%llu cr2=%llu cr3=%llu cr4=%llu cr8=%llu\n",
		     sregs.cr0, sregs.cr2, sregs.cr3, sregs.cr4, sregs.cr8);

	trace_printk("efer=%llu apic_base=%llu flags=%llu\n", sregs.efer,
		     sregs.apic_base, sregs.flags);

	trace_printk("pdptrs: %llu %llu %llu %llu\n", sregs.pdptrs[0],
...
static void dump_ftrace_vcpu_kvm_lapic_state(struct kvm_vcpu *vcpu)
{
	...
	trace_printk("APIC_TMR: %u %u %u %u %u %u %u %u\n",
	...
	trace_printk("APIC_IRR: %u %u %u %u %u %u %u %u\n",
...
static void dump_ftrace_vcpu_debugregs(struct kvm_vcpu *vcpu)
{
	...
	trace_printk("db[0]=%llu db[1]=%llu db[2]=%llu db[3]=%llu\n",
		     debugregs.db[0], debugregs.db[1], debugregs.db[2],
		     debugregs.db[3]);

	trace_printk("dr6=%llu dr7=%llu flags=%llu\n", debugregs.dr6,
